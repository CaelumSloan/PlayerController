Shader "Custom/EdgeRender"
{
	...
	Properties
	{
		...
		_Threshold("Threshold", Range(0.001,10)) = 0.001
			...
			*/
			...
			fixed4 frag(v2f i) : COLOR
			{
				...
				*/
				...
				//float threshold = a.w * _Threshold; //Less sensitive over distance
				...
				if (pixelDifference > threshold) 
				{
					...
					//return lerp(tex2D(_MainTex, i.screenPos.xy), _Color, (1-a.w * tex2D(_NoiseTex, i.screenPos.xy) )); //Lerp Color becomes closer to objects original color based on distance.
					//float4(aColor.zzz, 1);
